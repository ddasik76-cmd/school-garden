...
현재 날씨 화면에는 오늘의 실시간 날씨가 아니라 최근 확정 일자료가 표시됩니다. 일자료 발표 시점을 고려해 한국시간 오전 11시 이전에는 이틀 전, 이후에는 전날을 기본 조회합니다. 관찰 기록은 날씨 조회를 기다리지 않고 저장하며, 확정 자료가 없으면 `weatherStatus: "pending"`과 당시 `stationId`를 함께 저장합니다. 다음날 오전 11시 이후 앱 실행 또는 포그라운드 복귀 시 해당 날짜·관측소로 자동 조회합니다. 성공하면 그 기록의 `weather`와 `weatherStatus: "final"`만 갱신하고 학생 입력·사진·관찰 수정 시각은 유지합니다. API 또는 저장 실패 시 기록을 그대로 두고 다음 접속 때 재시도합니다. 앱이 닫힌 동안에는 조회하지 않습니다.

타임라인에는 대기 중이면 `🕒 기상자료 집계 중 · 다음날 11시 이후 자동 확정`, 확정되면 `✅ 기상청 확정 일자료`가 표시됩니다. 자동 갱신은 관측소가 고정된 명시적인 pending 기록에만 적용합니다. 상태 정보가 없는 기존 기록은 그대로 보존하며 현재 설정으로 과거 관측소를 추측하지 않습니다. localStorage `gardenClimateJournal.v1`, IndexedDB `gardenClimateJournalDB/photos`와 ZIP `data.json`/`photos/` 구조는 유지됩니다.
...
`node --test tests/*.test.cjs`로 JavaScript 문법, ASOS 응답, 한국시간 11시 경계, pending→final 전환, 관측소 고정, API/저장 실패 후 재시도, 수정·삭제·복원 중 동시 요청, 기존 날씨 보존, 저장 및 ZIP 호환성과 지역 검색을 검사합니다. GitHub Actions는 검증만 수행하며 앱 코드를 자동 치환하지 않습니다. 실제 기상청 서버 응답과 학교망 연결은 별도로 확인해야 합니다.
